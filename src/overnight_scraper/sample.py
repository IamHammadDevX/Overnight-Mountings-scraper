...
from .media import unrecognized_color_media
...
def run_sample(requirements: Path, output: Path, limit: int = 10, start: int = 0) -> list[FamilyResult]:
    ...
    for family in load_families(requirements)[start:start + limit]:
        ...
        def lookup(sku: str) -> Attempt:
            ...
            attempt = Attempt(family.family_id, sku, page.url, page.outcome, page.detail); attempts.append(attempt); return attempt
        source = find_family_source(family, lookup)
        if source.outcome is not LookupOutcome.FOUND:
            (output / f"{family.family_id}.json").write_text(json.dumps({"family_id": family.family_id, "attempts": [asdict(x) for x in attempts]}, indent=2), encoding="utf-8"); continue
        colors: dict[str, ColorMedia] = {}; unknown: list[str] = []
        for label, parameter in (("white", "White"), ("yellow_gold", "Yellow"), ("rose_gold", "Rose")):
            ...
            if page.html is not None: unknown = unrecognized_color_media(page.html)
            colors[label] = media if media is not None else ColorMedia(available=None, evidence={"error": page.detail, "url": page.url})
        result = FamilyResult(family.family_id, source.sku, source.url or "", colors["white"], colors["yellow_gold"], colors["rose_gold"], attempts, unknown)
        results.append(result); (output / f"{family.family_id}.json").write_text(json.dumps(result.as_dict(), indent=2), encoding="utf-8")
    return results
